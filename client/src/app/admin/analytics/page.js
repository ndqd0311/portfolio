...
export default function AdminAnalyticsPage() {
  ...
  const [activeTab, setActiveTab] = useState('blogs'); // 'blogs' or 'traffic'
  ...
  return (
    <div className="space-y-8 min-h-full flex flex-col pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex p-1 bg-surface-dark border border-white/5 rounded-xl self-start sm:self-center">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 text-xs font-mono rounded-lg transition-all cursor-pointer ${activeTab === 'blogs'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 text-xs font-mono rounded-lg transition-all cursor-pointer ${activeTab === 'traffic'
                ? 'bg-electric-cyan text-background font-bold shadow-lg shadow-electric-cyan/20'
                : 'text-on-surface-variant hover:text-on-surface'
              }`}
          >
      ...
      {/* --- TAB 1: THỐNG KÊ BLOG (DATABASE METRICS) --- */}
      {activeTab === 'blogs' && (
        <div className="space-y-8">
          ...
          {error && (
            <div className="glass-card rounded-2xl border border-red-500/10 p-8 bg-red-500/5 max-w-2xl mx-auto space-y-6">
              ...
              <button
                onClick={fetchStats}
                className="flex items-center gap-2 px-5 py-2.5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 text-xs font-mono text-on-surface transition-all active:scale-[0.98] cursor-pointer"
              >
      ...
      {activeTab === 'traffic' && (
        <div className="flex-1 flex flex-col min-h-[600px]">
          {shareUrl ? (
            /* If share link exists, render the live Umami statistics */
            <div className="relative flex-1 min-h-[650px] rounded-2xl overflow-hidden border border-white/10 bg-black/25 shadow-2xl glass-card">
              {iframeLoading && (
                <div className="absolute inset-0 flex flex-col justify-center items-center font-mono text-on-surface-variant bg-background/50 backdrop-blur-sm z-10">
                  <span className="material-symbols-outlined text-electric-cyan text-4xl animate-spin mb-4">sync</span>
                  <span>Loading Live Visitors Dashboard...</span>
                </div>
              )}
              <iframe
                src={cleanIframeUrl}
                onLoad={() => setIframeLoading(false)}
                className="w-full h-full min-h-[650px] border-0 rounded-2xl"
                allowFullScreen
                loading="lazy"
              />
            </div>
          ) : (
            /* If no share link exists, display setup instructions nicely */
            <div className="glass-card rounded-2xl border border-white/5 p-8 max-w-2xl mx-auto space-y-6 bg-surface-dark/20">
              <div className="flex items-center gap-3 text-electric-cyan">
                <span className="material-symbols-outlined text-3xl">info</span>
                <h3 className="font-sora text-lg font-bold">Web Traffic Tracking Required</h3>
              </div>

              <div className="font-sans text-sm text-on-surface-variant leading-relaxed space-y-4">
                <p>
                  To view real-time traffic statistics (total visitors, views, sessions, bounce rates, location country, and browser device metrics), you need to configure the Umami Share URL.
                </p>
                <p className="font-mono text-xs text-electric-cyan/80">
                  Note: The frontend tracking code is already active. You only need to add your environment variables on Vercel to activate the live traffic dashboard view.
                </p>
              </div>
              ...
              <div className="border-t border-white/5 pt-6 space-y-4">
                ...
                <ol className="list-decimal list-inside font-sans text-xs text-on-surface-variant space-y-3 leading-relaxed">
                  ...
                  <li>
                    Add the following keys to your Vercel settings (or local <code className="bg-white/5 px-1.5 py-0.5 rounded font-mono text-[11px]">.env.local</code>):
                  ...
                  <li>Redeploy or restart your local server.</li>
                </ol>
              </div>
            </div>
          )}
